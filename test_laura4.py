"""
test_laura4.py — Nivel 4: Suite Completa 30 Escenarios
Laura 5.2 — Boats4U

Cómo correr:
    python test_laura4.py

Salida: resultado_test_laura4.txt
"""
...
OUTPUT  = "resultado_test_laura4.txt"
SYSTEM_FILE = "LAURA_5_2.md"
...
ESCENARIOS = [
    ...
    {
        "id": "A2", "nombre": "Saludo en inglés — respuesta 100% inglés",
        "turnos": [
            {"input": "Hi! I want to book a boat in Cartagena",
             "criterios": [
                ("ingles_saludo",  False, r"good (morning|afternoon|evening)",             "Saludo en inglés"),
                ("no_espanol",     True,  r"\b(hola|buenas|las|los|para|con)\b",           "Sin mezcla de español"),
    ...
    {
        "id": "A3", "nombre": "Cliente con intención clara desde inicio — no activa pregunta de elección",
        "turnos": [
    ...
    {
        "id": "A4", "nombre": "Mensaje vacío / ininteligible",
        "turnos": [
            {"input": "...",
             "criterios": [
    ...
    {
        "id": "A5", "nombre": "Cliente pregunta qué es Boats4U",
        "turnos": [
            {"input": "¿Qué es Boats4U?",
             "criterios": [
                ("describe",       False, r"cartagena|experiencia|náutic|mar|bote",       "Describe la empresa"),
    ...
    # ─── BLOQUE C: CAMBIOS EN MID-CONVERSACIÓN ───────────────────────────────
    ...
    {
        "id": "C4", "nombre": "Pide más espacio — cascada de botes",
        "turnos": [
            ...
            {"input": "¿Tienen algo más grande y cómodo?",
             "criterios": [
                ...
                ("no_yate",        True,  r"azimut|leopard",                              "No salta a yate sin señal premium"),
                ("una_opcion",     True,  r"bravo 380.{0,100}bravo 410",                  "Una sola opción"),
            ]}
        ]
    },
    ...
    {
        "id": "D1", "nombre": "Señal de lujo — Azimut 70 primero",
        "turnos": [
            {"input": "Somos 2, queremos algo muy lujoso y exclusivo para nuestro aniversario",
             "criterios": [
                ...
                ("no_menu",        True,  r"azimut 70.{0,200}azimut 62",                  "No presenta múltiples yates"),
            ]}
        ]
    },

    {
        "id": "D2", "nombre": "Cascada premium — baja solo si cliente pide",
        "turnos": [
            {"input": "Somos 4, queremos algo exclusivo",                       "criterios": []},
            {"input": "¿Tienen algo más íntimo?",
             "criterios": [
                ("baja_nivel",     False, r"azimut 62|azimut 58|leopard",                 "Baja un nivel por petición"),
                ("no_salta_dos",   True,  r"azimut 55",                                   "No salta dos niveles"),
            ]}
        ]
    },

    {
        "id": "D3", "nombre": "Señal de celebración — despedida de soltero",
        "turnos": [
            {"input": "Somos 8, es una despedida de soltero, queremos algo épico",
             "criterios": [
                ("activa_premium", False, r"azimut|leopard|exclusiv|premium|lujo",        "Activa opción premium"),
                ("no_firpol_solo", True,  r"^(?!.*azimut|.*leopard).*firpol",             "No queda solo en bote estándar"),
            ]}
        ]
    },

    {
        "id": "D4", "nombre": "No ofrece yate sin señal — grupo normal",
        "turnos": [
            {"input": "Somos 5 para Islas del Rosario",
             "criterios": [
                ("bravo_290",      False, r"bravo 290",                                   "Recomienda bote base"),
                ("no_yate",        True,  r"azimut|leopard",                              "No ofrece yate sin señal"),
            ]}
        ]
    },

    # ─── BLOQUE E: MANEJO DE PRECIOS Y MONEDA ────────────────────────────────

    {
        "id": "E1", "nombre": "Precio en COP para cliente español",
        "turnos": [
    ...
    {
        "id": "E2", "nombre": "Precio en USD para cliente inglés",
        "turnos": [
        ...
        "turnos": [
            {"input": "¿Cuánto cuesta el Bravo 410 para Islas del Rosario?",
             "criterios": [
                ...
                ("no_improvisa",   True,  r"aproximad|alrededor|más o menos|depende",     "No improvisa ni estima"),
            ]}
        ]
    },
    ...
    {
        "id": "F2", "nombre": "Fecha ambigua — pide exactitud",
        "turnos": [
            {"input": "Somos 6 para Islas del Rosario",    "criterios": []},
            {"input": "Queremos ir el próximo sábado",
             "criterios": [
                ("pide_exacta",    False, r"d[ií]a|mes|fecha exacta|cu[aá]l s[aá]bado|\?", "Pide día y mes exactos"),
    ...
    {
        "id": "F3", "nombre": "Fecha futura válida — avanza directo",
        "turnos": [
            {"input": "Somos 4 para Golden Hour",          "criterios": []},
            {"input": "Queremos ir el 15 de mayo de 2026",
             "criterios": [
                ("acepta",         False, r"mayo|15|2026|perfect|genial|excelente",       "Acepta la fecha"),
                ("avanza",         False, r"nombre|correo|confirm|reserv|pago",           "Avanza hacia confirmación"),
            ]}
        ]
    },

    {
        "id": "F4", "nombre": "Fecha pasada en inglés",
        "turnos": [
            {"input": "We are 4 for the Golden Hour",      "criterios": []},
            {"input": "We want to go on January 10th 2023",
             "criterios": [
                ("rechaza_en",     False, r"past|already|2023|future|upcoming",           "Rechaza en inglés"),
    ...
    {
        "id": "G1", "nombre": "Primera objeción — Nivel 1 o 2, sin descuento",
        "turnos": [
            {"input": "Somos 4 para Islas del Rosario",    "criterios": []},
            {"input": "Está muy caro",
             "criterios": [
                ("cierre_n1_n2",   False, r"asegur|env[ií]o.*link|hoy|link",              "Activa Nivel 1 o 2"),
                ("no_descuento",   True,  r"10\s*%|10 por ciento",                        "Sin descuento todavía"),
    ...
    {
        "id": "G2", "nombre": "Segunda objeción — activa descuento 10%",
        "turnos": [
            {"input": "Somos 4 para Islas del Rosario",    "criterios": []},
    ...
    {
        "id": "G3", "nombre": "Déjame pensarlo — activa Nivel 2 directo",
        "turnos": [
    ...
    {
        "id": "G4", "nombre": "Rechazo explícito — cierra con elegancia",
        "turnos": [
            {"input": "Somos 4 para Islas del Rosario",    "criterios": []},
            {"input": "No gracias, no me interesa",
             "criterios": [
                ("cierra_bien",    False, r"\w{20,}",                                     "Responde coherentemente"),
                ("no_insiste",     True,  r"asegur|link|descuento|10\s*%",                "No insiste ante rechazo explícito"),
                ("no_prohibidas",  True,  r"procedemos|le informo|en qu[eé] m[aá]s",      "Sin frases prohibidas"),
            ]}
        ]
    },
    ...
    {
        "id": "G5", "nombre": "Objeción en inglés — niveles en inglés",
        "turnos": [
            {"input": "We are 4 for the Rosario Islands",  "criterios": []},
            {"input": "That's too expensive for us",
             "criterios": [
                ("cierre_en",      False, r"today|secure|lock|link|recommend",            "Activa cierre en inglés"),
                ("no_descuento",   True,  r"10\s*%|10 percent|discount",                  "Sin descuento todavía"),
    ...
    {
        "id": "H4", "nombre": "Lead desde landing — datos completos",
        "turnos": [
    ...
    {
        "id": "H5", "nombre": "Confirmación de reserva — resumen correcto",
        "turnos": [
            {"input": "Somos 6 para Islas del Rosario",    "criterios": []},
            ...
            {"input": "Mi nombre es Ana Gómez, correo ana@gmail.com",
             "criterios": [
                ...
                ("link_pago",      False, r"link|pago",                                   "Envía link de pago"),
                ("politica_50",    False, r"50\s*%|50 por ciento",                        "Menciona política 50/50"),
...
def main():
    print(f"\n{'='*65}")
    print(f"🚤  BOATS4U — Suite 30 Escenarios Laura 5.2")
    ...
    try:
        ...
        print(f"\n❌  No se encontró {SYSTEM_FILE}")
        return

    lineas = []
    lineas.append("=" * 65)
    lineas.append("BOATS4U — SUITE COMPLETA 30 ESCENARIOS LAURA 5.2")
    ...
    for esc in ESCENARIOS:
        ...
        if ultima_resp:
            lineas.append(f"\n  Última respuesta:")
            lineas.append(f"  {ultima_resp[:350]}{'...' if len(ultima_resp)>350 else ''}")
    ...
    lineas.append(f"{'='*65}")
    nombres_bloque = {
        "A": "Activación y saludo",
        "B": "Tabla de pasajeros",
        "C": "Cambios mid-conversación",
    ...
    for b, stats in bloques.items():
        ...
        lineas.append(f"  {b} — {nombres_bloque.get(b,''):<30} {stats['pass']}/{total_b}")
