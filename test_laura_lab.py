"""
test_laura_lab.py — Suite Lab: 45 Escenarios contra LAURA PROD 1.0
Boats4U — Versión lab para validación de producción

Cambios respecto a test_laura5.py:
  - SYSTEM_FILE apunta a LAURA_PROD_1_0.md
  - OUTPUT apunta a resultado_lab_prod10.txt
  - CHECKPOINT propio para no mezclar con runs anteriores
  - Header actualizado (PROD 1.0)

Cómo correr:
    python test_laura_lab.py
    python test_laura_lab.py --desde D3
    python test_laura_lab.py --modelo claude-sonnet-4-6

Salida: resultado_lab_prod10.txt
"""
...
OUTPUT      = "resultado_lab_prod10.txt"
CHECKPOINT  = "checkpoint_lab_prod10.json"
SYSTEM_FILE = "LAURA_PROD_1_0.md"
...
def cargar_prompt():
    with open(SYSTEM_FILE, encoding="utf-8") as f:
        content = f.read()
    content = content.replace("{{CURRENT_DATETIME}}", "2026-03-16 20:45")
    return content + "\n\n" + PRECIOS_DUMMY + "\n\n" + MOCK_REGISTRAR_LEAD
...
ESCENARIOS = [
    ...
    # ─── BLOQUE J: COBERTURA ESPECIFICA PROD 1.0 ──────────────────────────────

    {
        "id": "J1", "nombre": "No infiere fin de semana",
        "turnos": [
            {"input": "Somos 4 para Golden Hour",          "criterios": []},
    ...
    {
        "id": "J2", "nombre": "No infiere manana",
        "turnos": [
            {"input": "Somos 6 para Islas del Rosario",    "criterios": []},
            {"input": "Queremos ir mañana",
             "criterios": [
                ("pide_exacta",    False, r"d[ií]a|mes|fecha exacta|\?",                  "Pide dia y mes exactos"),
                ("no_infiere",     True,  r"17 de marzo|marzo 17",                        "No calcula manana como fecha"),
            ]}
        ]
    },

    {
        "id": "J3", "nombre": "Segunda objecion en ingles activa descuento",
        "turnos": [
            {"input": "We are 4 for the Rosario Islands",  "criterios": []},
    ...
    {
        "id": "J4", "nombre": "Cumpleanos activa premium antes que bote base",
        "turnos": [
    ...
    {
        "id": "J5", "nombre": "Bajada gradual desde Azimut 62",
        "turnos": [
...
def main():
    parser = argparse.ArgumentParser(description="Suite Lab — LAURA PROD 1.0")
    ...
    print(f"\n{'='*65}")
    print(f"BOATS4U — Suite Lab 45 Escenarios LAURA PROD 1.0")
    ...
    lineas = []
    lineas.append("=" * 65)
    lineas.append("BOATS4U — SUITE LAB 45 ESCENARIOS LAURA PROD 1.0")
